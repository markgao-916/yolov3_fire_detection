...
class Dataset(object):
    ...
    def parse_annotation(self, annotation):
        ...
        bboxes = np.array([list(map(self.convert_to_zero, box.split(','))) for box in line[1:]])
        ...
        for bbox in bboxes:
            ...
            for i in range(3):
                ...
                if np.any(iou_mask):
                    ...
                    exist_positive = True
                # ignore_iou_mask = (iou_scale<0.3)*(iou_scale>0.1)
                # if np.any(ignore_iou_mask):
                #     xind, yind = np.floor(bbox_xywh_scaled[i, 0:2]).astype(np.int32)
                #     label[i][yind, xind, iou_mask, :] = 0
                #     label[i][yind, xind, iou_mask, 4:5] = 1.0
